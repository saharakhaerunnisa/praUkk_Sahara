...
if(isset($_GET['kembalikan'])) {
    $id = $_GET['kembalikan'];
    mysqli_query($connect, "UPDATE buku SET status='Tersedia' WHERE id='$id'");
...
<body>
  ...
  <nav class="navbar">
    ...
    <span class="navbar-siswa-info">Portal Siswa</span>

    <div class="navbar-end">
      <div class="student-chip">
        <div class="chip-info">
          <span class="chip-name"><?php echo $user; ?></span>
        </div>
      </div>
            ...
            <tr>
              ...
              <td>
                <div class="action-btns">
                  <button class="btn-icon" title="Edit"><a href="dashboard.php?kembalikan=<?= $row['id']?>"
                  onclick="return confirm('Apakah Anda yakin ingin mengembalikan buku ini?');">✏️</a></button>
                  <button class="btn-icon" title="Pinjam" 
                        onclick="openModal('<?= $row['id'] ?>', '<?= htmlspecialchars($row['judul']) ?>', '<?= htmlspecialchars($row['penulis']) ?>')">
                    📚
                </button>
                </div>
              </td>
             ...
             <div class="modal-overlay" id="modalPinjam">
              <div class="modal-box">
                <div class="modal-title">Konfirmasi Peminjaman</div>
                <div class="modal-sub">Pastikan data buku sudah benar sebelum mengajukan peminjaman.</div>
                
                <div id="modalBookDetail" style="margin-bottom: 1rem; padding: 10px; background: #1e293b; border-radius: 8px;">
                    </div>

                <div style="font-size: 12px; color: var(--text-muted); margin-bottom: 1.5rem; line-height: 1.6;">
                  📅 Durasi peminjaman: <strong style="color: var(--text-light);">7 hari</strong><br>
                  👤 Peminjam: <strong style="color: var(--text-light);"><?php echo $_SESSION['username']; ?></strong><br>
                  ⚠️ Keterlambatan pengembalian dikenakan denda Rp1.000/hari.
                </div>
                
                <div class="modal-footer">
                  <button class="btn-cancel" onclick="closeModal()">Batal</button>
                  <a href="#" id="linkKonfirmasiPinjam" class="btn-confirm" style="text-decoration: none; display: inline-block; padding: 10px 20px; background: #38BDF8; color: #0F172A; border-radius: 5px; font-weight: bold;">
                    ✔ Ajukan Pinjam
                  </a>
                </div>
              </div>
